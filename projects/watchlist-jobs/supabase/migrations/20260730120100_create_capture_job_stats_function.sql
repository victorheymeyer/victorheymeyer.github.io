-- Computes one day's row of job_stats. jobs_location_flags is already
-- filtered to the latest snapshot_date, so every row in it is by definition
-- an "active" job; post_status in ('New','New*') marks the "new" subset
-- (the spec's New Flag OR New* Flag). maybe_wa/maybe_remote_wa are the
-- Seattle/RemoteWA flags; their conjunction covers the Seattle_AND_RemoteWA
-- rows without needing the stored seattle_and_remote column (that column
-- lives on job_content, not on the view).
...
begin
  ...
  from
    (select count(distinct watchlist_company) as n from job_content) v_universe,
    (
      select
...
revoke execute on function capture_job_stats() from public;
grant execute on function capture_job_stats() to service_role;
