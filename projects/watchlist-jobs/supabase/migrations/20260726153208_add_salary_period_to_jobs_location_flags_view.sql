-- Expose the new job_content.salary_period column through the view the
-- frontend actually queries. Appended as the last column so CREATE OR
-- REPLACE VIEW can add it without dropping/re-granting (existing columns
-- keep their name/type/order).
