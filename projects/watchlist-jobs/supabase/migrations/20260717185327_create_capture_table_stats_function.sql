...
begin
  execute format('select count(*) from %s', v_reg) into v_rows;
  ...
  if p_date_col is not null then
    execute format('select min(%I)::date, max(%I)::date from %s',
                   p_date_col, p_date_col, v_reg)
      into v_mind, v_maxd;
  end if;
  ...
  values (v_today, p_table, v_rows, v_total, v_heap,
  ...
  loop
    ...
    values (v_today, p_table, v_col.column_name, v_col.ordinal_position,
...
revoke execute on function capture_table_stats(text, text) from public;
