-- capture_job_stats() derived v_today from (now() at time zone 'utc')::date,
-- stamping job_stats.snapshot_date (a Seattle-day column per the project's
-- timezone invariant) with a UTC calendar day instead. Only this line
-- changes from the current definition in
-- 20260805130000_scope_job_stats_universe_to_active_watchlist.sql.
